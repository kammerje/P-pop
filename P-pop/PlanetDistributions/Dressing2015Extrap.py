...
class PlanetDistribution():
    ...
    def __init__(self,
        ...
        if (Scenario == 'baseline'):
            self.Rates = np.array([[1.38, 8.42, 20.59, 0., 0., 0.], [1.95, 9.94, 0., 26.85, 28.85, 0.], [0.41, 4.15, 0., 24.59, 19.98, 0.], [0., 2.72, 18.73, 27.58, 18.08, 0.], [0., 1.59, 8.29, 14.51, 8.61, 0.], [0., 0.65, 3.25, 3.37, 1.97, 0.], [0., 0.38, 1.05, 0.56, 0., 0.]])*1e-2
        elif (Scenario == 'pessimistic'):
            self.Rates = np.array([[1.38, 8.42, 20.59, 0., 0., 0.], [1.95, 9.94, 0., 26.85, 28.85, 0.], [0.41, 4.15, 0., 24.59, 19.98, 0.], [0., 2.72, 18.73, 27.58, 18.08, 0.], [0., 1.59, 8.29, 14.51, 8.61, 0.], [0., 0.65, 3.25, 3.37, 1.97, 0.], [0., 0.38, 1.05, 0.56, 0., 0.]])*1e-2
            self.Rates -= np.array([[0.53, 2.39, 5.57, 0., 0., 0.], [0.61, 2.13, 0., 6.79, 10.34, 0.], [0.2, 1.28, 0., 6., 7.42, 0.], [0., 1.01, 3.95, 6.31, 6.57, 0.], [0., 0.69, 2.55, 4.62, 3.84, 0.], [0., 0.32, 1.37, 1.62, 0.85, 0.], [0., 0.19, 0.52, 0.19, 0., 0.]])*1e-2
        elif (Scenario == 'optimistic'):
            self.Rates = np.array([[1.38, 8.42, 20.59, 0., 0., 0.], [1.95, 9.94, 0., 26.85, 28.85, 0.], [0.41, 4.15, 0., 24.59, 19.98, 0.], [0., 2.72, 18.73, 27.58, 18.08, 0.], [0., 1.59, 8.29, 14.51, 8.61, 0.], [0., 0.65, 3.25, 3.37, 1.97, 0.], [0., 0.38, 1.05, 0.56, 0., 0.]])*1e-2
            self.Rates += np.array([[0.93, 3.53, 8.7, 0., 0., 0.], [0.93, 2.82, 0., 10.7, 28.66, 0.], [0.51, 1.94, 0., 9.08, 16.07, 0.], [0., 1.73, 5.39, 9.42, 13.21, 0.], [0., 1.39, 3.96, 7.71, 9.78, 0.], [0., 1., 2.72, 4.62, 5.87, 0.], [0., 0.77, 1.82, 2.32, 0., 0.]])*1e-2
        elif (Scenario == 'mc'):
        ...
        self.BinsPorb = np.log(np.array([0.5, 1.7, 5.5, 18.2, 60.3, 200., 400.])) # d
        ...
        # S = Star.Star(Name='Sun', Dist=10., Stype='G', Rad=Rsun, Teff=Tsun, Mass=Msun, RA=0., Dec=0.)
        # Ndraw = 100000
        # Rps = []
        # Fps = []
        # for i in range(Ndraw):
        #     Rp, Fp = self.draw(Rp_range=[0.5, 1.0], Porb_range=[0.5, 1.7], Star=S)
        #     Rps += [Rp]
        #     Fps += [Fp]
    ...
    def SummaryPlot(self,
        ...
        plt.gca().plot_surface(np.log10(np.exp(self.xx)), np.log10(np.exp(self.yy)), np.log10(np.exp(self.zz)))
        plt.gca().plot_wireframe(np.log10(np.exp(self.xx)), np.log10(np.exp(self.yy)), np.log10(np.exp(self.r_bqf)), color='black')
        plt.gca().set_xlabel(r'log${}_{10}$($P_{\rm{orb}}$)')
        plt.gca().set_ylabel(r'log${}_{10}$($R_{\rm{p}}$)')
        plt.gca().set_zlabel(r'log${}_{10}$($\eta$)')
