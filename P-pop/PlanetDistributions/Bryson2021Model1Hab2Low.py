...
class PlanetDistribution():
    ...
    def __init__(self,
        ...
        # Print.
        print('--> Initializing Bryson2021Model1Hab2Low planet distribution')
        ...
        self.F0s = np.array([1.11, 0.88, 0.44]) * 0.952 # normalization because hab2 stars average is not equal to the Sun
        ...
        # # amin = np.sqrt(L/(4*np.pi)/Fmax/1361.)
        # # Pmin = np.sqrt(4.*np.pi**2/(G*M)*amin**3) / 3600. / 24.
        # # amax = np.sqrt(L/(4*np.pi)/Fmin/1361.)
        # # Pmax = np.sqrt(4.*np.pi**2/(G*M)*amax**3) / 3600. / 24.
        
        # C1 = 0.242
        # def func(R, F):
        #     return self.F0 * C1 * R**self.alpha * F**self.beta * (T/self.Tsun)**self.gamma * g(T)
        # F0 = dblquad(func, Fmin, Fmax, 0.5, 1.5)
        
        # Teffs = np.linspace(3900., 6200., 100)
        # etas = []
        # for Teff in Teffs:
        #     S = Star.Star(Name='Sun', Dist=10., Stype='G', Rad=Rsun, Teff=Teff, Mass=Msun, RA=0., Dec=0.)
        #     Ndraw = 100000
        #     Rps = []
        #     Fps = []
        #     for i in range(Ndraw):
        #         Rp, Fp = self.draw(Rp_range=[0.5, 1.5], Fp_range=[Fmin, Fmax], Star=S)
        #         Rps += [Rp]
        #         Fps += [Fp]
        ...
        # plt.ylim([0., 4.])
        # plt.xlabel('Host star effective temperature [K]', size=14)
        # plt.ylabel('HZ occurrence rate', size=14)
        # plt.show()
    ...
    def SummaryPlot(self,
        ...
        plt.suptitle('Bryson2021Model1Hab2Low')
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        if (FigDir is not None):
            plt.savefig(FigDir+'PlanetDistribution_Bryson2021Model1Hab2Low.pdf')
