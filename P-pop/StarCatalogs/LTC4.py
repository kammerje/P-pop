...
class StarCatalog():
    ...
    def __init__(self,
                 ...
                 Path=os.path.join(os.path.split(os.path.abspath(__file__))[0], 'LTC4.csv')):
        """
        Parameters
        ----------
        Stypes: list
    ...
    def read(self,
        ...
        # Print.
        print('--> Reading star catalog LTC4.csv')
        ...
        with open(Path) as csvfile:
            ...
            for i, row in enumerate(SC_in):
                if (i == 0):
                    ...
                    #
                    row = np.array(row)
                    ColName = np.where(row == 'main_id')[0][0]
                    ColDist = np.where(row == 'dist_st_value')[0][0]
                    ColStype = np.where(row == 'sptype_string')[0][0]
                    ColRad = np.where(row == 'radius_st_value')[0][0]
                    # ColTeff = np.where(row == 'teff_st_value')[0][0]
                    ColTeff = np.where(row == 'teff')[0][0]
                    ColMass = np.where(row == 'mass_st_value')[0][0]
                    ColRA = np.where(row == 'coo_ra')[0][0]
                    ColDec = np.where(row == 'coo_dec')[0][0]
#                    ColVmag = np.where(row == 'st_vmag')[0][0]
#                    ColJmag = np.where(row == 'st_j2m')[0][0]
#                    ColHmag = np.where(row == 'st_h2m')[0][0]
                    ColWDSsep = np.where(row == 'binary_flag')[0][0]
#                    ColWDSdmag = np.where(row == 'wds_deltamag')[0][0]
                    CollGal = np.where(row == 'coo_gal_l')[0][0]
                    ColbGal = np.where(row == 'coo_gal_b')[0][0]
